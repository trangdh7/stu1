...
try {
    ...
    if ($tcpTest.TcpTestSucceeded) {
        ...
        Write-Host "    Có thể do firewall chặn port $smtpPort (outbound)" -ForegroundColor Yellow
        Write-Host "    Giải pháp: Mở port $smtpPort (outbound) trên firewall" -ForegroundColor Yellow
    }
} catch {
...
if ($firewallRules) {
    ...
    Write-Host "     Không tìm thấy firewall rule cho SMTP port $smtpPort" -ForegroundColor Yellow
...
if ([string]::IsNullOrEmpty($emailPassword)) {
    Write-Host "     Không tìm thấy biến môi trường EmailSettings__FromPassword" -ForegroundColor Yellow
    Write-Host "    Kiểm tra appsettings.json hoặc appsettings.Development.json có FromPassword không rỗng" -ForegroundColor Cyan
} else {
...
Write-Host "2. Kiểm tra appsettings.json hoặc appsettings.Development.json có đúng cấu hình không" -ForegroundColor White
