...
namespace Webkho_20241021.Areas.Giamdoc.Controllers
{
    ...
    public class YeucauController : Controller
    {
        ...
        public IActionResult GetVTYeucau(string MaYeucau)
        {
            ...
            if (isNhapKhoRequest)
            {
                ...
                var result2 = vatTuList.Select(v =>
                {
                    ...
                    return new { v.ID, v.VTMaYeucau, v.TenSanpham, v.MaSanpham, v.YCMakho, v.HangSX, v.NhaCC, v.SLCu, v.SLMoi, v.SL, v.DonVi, v.NgayCanHang, v.NgayNhapkho, v.NgayBaohanh, v.ThoiGianBH, v.NgayDuyet, v.TrangThai, v.GhiChu, TonKho = tonKho, SlThieu = Math.Max(0, (v.SLMoi ?? v.SL ?? 0) - tonKho), SlDaXuat = (int?)null };
                }).ToList();
                return Json(result2);
            }
        ...
        /// <summary>
        /// Lấy danh sách vật tư gộp từ nhiều mã yêu cầu (chỉ áp dụng cho yêu cầu vật tư gốc vtyeucau).
        ...
        public IActionResult GetVTYeucauMulti(string maYeucauList)
        {
            ...
            // Chỉ gom các yêu cầu có dòng chi tiết trong vtyeucau
            var vatTuList = _context.vtyeucau
            ...
            var processedVatTuList = vatTuList.Select(v =>
            {
                ...
                var slDaXuatThucTe = !string.IsNullOrWhiteSpace(v.MaSanpham)
                    ? YeucauUpdateHelper.TinhSoLuongDaCap(_context, maYc, maSp)
                    : 0;

                var slThieu = YeucauUpdateHelper.TinhSoLuongConThieuTheoMaYeuCauCoBan(_context, maYc, maSp);

                var isDaXuatKho = (v.TrangThai ?? "").IndexOf("Đã xuất kho", StringComparison.OrdinalIgnoreCase) >= 0;
                var slDaXuat = slDaXuatThucTe > 0
                    ? (int?)slDaXuatThucTe
        ...
        public IActionResult ExportYeucauVatTuExcel(string MaYeucau)
        {
            ...
            if (hasVatTuYeuCau)
            {
                // Sort giống dữ liệu đang hiển thị trên màn hình (ưu tiên "Chờ giám đốc duyệt", sau đó theo Mã VT)
                bool IsAwaitingDirectorStatusForExport(string? status)
                {
                    ...
                    return normalized == "chờ giám đốc duyệt"
                        ...
                        || normalized.Contains("chờ giám đốc");
                }

                var vatTuListRaw = _context.vtyeucau.Where(v => v.VTMaYeucau == MaYeucau).ToList();
                var vatTuList = vatTuListRaw
                    .OrderByDescending(v => IsAwaitingDirectorStatusForExport(v.TrangThai))
                    .ThenBy(v => (v.MaSanpham ?? "").Trim(), StringComparer.CurrentCultureIgnoreCase)
                    .ToList();
                ...
                foreach (var v in vatTuList)
                {
                    ...
                    // Ngày duyệt hiển thị trên màn hình: chỉ show khi trạng thái đã qua bước duyệt
                    var ngayDuyetDisplay = (v.TrangThai == "Đã duyệt" ||
                                           v.TrangThai == "Đang mua hàng" ||
                                           v.TrangThai == "Chờ xuất kho" ||
                                           v.TrangThai == "Đã xuất kho" ||
            ...
            using (var package = new ExcelPackage())
            {
                ...
                {
                    // TT trên màn hình: nếu TT rỗng thì fallback STT tăng dần
                    object? ttValue = r.TT;
                    if (ttValue is string ttStr && string.IsNullOrWhiteSpace(ttStr))
                    {
                        ttValue = null;
                    }
                    worksheet.Cells[row, 1].Value = ttValue ?? stt++;
        ...
        /// <summary>
        /// Xuất Excel danh sách vật tư gộp từ nhiều mã yêu cầu (dùng cho Giám đốc khi chọn nhiều yêu cầu).
        /// </summary>
        [HttpGet]
        public IActionResult ExportYeucauVatTuExcelMulti(string maYeucauList)
        {
            if (string.IsNullOrWhiteSpace(maYeucauList))
            {
                return NotFound();
            ...
            foreach (var v in vatTuList)
            {
                ...
                var slDaXuatThucTe = !string.IsNullOrWhiteSpace(maSp)
                    ? YeucauUpdateHelper.TinhSoLuongDaCap(_context, maYc, maSp)
                    : 0;
                ...
                exportRows.Add(new
                {
                    // Màn hình gộp đang đánh số TT tăng dần theo thứ tự hiển thị, không dùng TT trong DB
                    TT = (object?)null,
                    v.TenSanpham,
                    v.MaSanpham,
                    // Giữ lại cả mã yêu cầu và mã dự án để có thể dùng khi cần
                    MaYeucau = maYc,
                    MaDuan = maDuan,
                    v.HangSX,
                    v.NhaCC,
            ...
            // Sort giống màn hình gộp (JS đang sort theo Mã VT trước khi render)
            exportRows = exportRows
                .OrderBy(r => ((r.MaSanpham as string) ?? "").Trim(), StringComparer.CurrentCultureIgnoreCase)
                .ThenBy(r => ((r.MaYeucau as string) ?? "").Trim(), StringComparer.CurrentCultureIgnoreCase)
                .ToList();
            ...
            using (var package = new ExcelPackage())
            {
                ...
                // Hiển thị danh sách mã yêu cầu đã gộp ngay trên tiêu đề, giống thanh thông tin trên màn hình
                var joinedMaList = string.Join(", ", maList);
                worksheet.Cells[1, 1].Value = string.IsNullOrWhiteSpace(joinedMaList)
                    ? "DANH SÁCH VẬT TƯ - GỘP NHIỀU YÊU CẦU"
                    : $"DANH SÁCH VẬT TƯ - GỘP NHIỀU YÊU CẦU: {joinedMaList}";
                worksheet.Cells[1, 1].Style.Font.Bold = true;
                worksheet.Cells[1, 1].Style.Font.Size = 14;
                worksheet.Cells[1, 1].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                worksheet.Cells[1, 1].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                ...
                worksheet.Cells[headerRow1, 2].Value = "Tên thiết bị / hàng hóa";
                worksheet.Cells[headerRow1, 3].Value = "Mã VT";
                // Hiển thị "Mã yêu cầu" đúng như trên màn hình gộp
                worksheet.Cells[headerRow1, 4].Value = "Mã yêu cầu";
                // Giữ thêm "Mã dự án" theo yêu cầu
                worksheet.Cells[headerRow1, 5].Value = "Mã dự án";
                worksheet.Cells[headerRow1, 6].Value = "Hãng SX";
                worksheet.Cells[headerRow1, 7].Value = "NCC";
                worksheet.Cells[headerRow1, 8, headerRow1, 12].Merge = true;
                worksheet.Cells[headerRow1, 8].Value = "SL";
                worksheet.Cells[headerRow1, 13].Value = "ĐV";
                worksheet.Cells[headerRow1, 14].Value = "Ngày cần";
                worksheet.Cells[headerRow1, 15].Value = "Ngày có hàng";
                worksheet.Cells[headerRow1, 16].Value = "Trạng thái";
                worksheet.Cells[headerRow1, 17].Value = "Ghi chú";
                worksheet.Cells[headerRow1, 18].Value = "Ngày duyệt";

                worksheet.Cells[headerRow2, 8].Value = "Cũ";
                worksheet.Cells[headerRow2, 9].Value = "Mới";
                worksheet.Cells[headerRow2, 10].Value = "Thiếu";
                worksheet.Cells[headerRow2, 11].Value = "Đã xuất";
                worksheet.Cells[headerRow2, 12].Value = "Tồn kho";
                ...
                worksheet.Cells[headerRow1, 7, headerRow2, 7].Merge = true;
                worksheet.Cells[headerRow1, 13, headerRow2, 13].Merge = true;
                worksheet.Cells[headerRow1, 14, headerRow2, 14].Merge = true;
                worksheet.Cells[headerRow1, 15, headerRow2, 15].Merge = true;
                worksheet.Cells[headerRow1, 16, headerRow2, 16].Merge = true;
                worksheet.Cells[headerRow1, 17, headerRow2, 17].Merge = true;
                worksheet.Cells[headerRow1, 18, headerRow2, 18].Merge = true;

                for (int r = headerRow1; r <= headerRow2; r++)
                {
                    using (var range = worksheet.Cells[r, 1, r, 18])
                    {
                ...
                foreach (var r in exportRows)
                {
                    ...
                    worksheet.Cells[row, 2].Value = r.TenSanpham ?? "";
                    worksheet.Cells[row, 3].Value = r.MaSanpham ?? "";
                    // Cột 4: Mã yêu cầu (giống trên màn hình)
                    worksheet.Cells[row, 4].Value = r.MaYeucau ?? "";
                    // Cột 5: Mã dự án (giữ thêm theo yêu cầu)
                    worksheet.Cells[row, 5].Value = r.MaDuan ?? "";
                    worksheet.Cells[row, 6].Value = r.HangSX ?? "";
                    worksheet.Cells[row, 7].Value = r.NhaCC ?? "";
                    worksheet.Cells[row, 8].Value = r.SLCu ?? 0;
                    worksheet.Cells[row, 9].Value = r.SLMoi ?? 0;
                    worksheet.Cells[row, 10].Value = r.SlThieu ?? 0;
                    worksheet.Cells[row, 11].Value = r.SlDaXuat != null ? r.SlDaXuat : "-";
                    worksheet.Cells[row, 12].Value = r.TonKho ?? 0;
                    worksheet.Cells[row, 13].Value = r.DonVi ?? "";
                    worksheet.Cells[row, 14].Value = r.NgayCanHang != null ? ((DateTime)r.NgayCanHang).ToString("dd/MM/yyyy") : "";
                    var ngayCoHangDispMulti = (r as dynamic)?.NgayCoHangDisplay as string;
                    worksheet.Cells[row, 15].Value = !string.IsNullOrEmpty(ngayCoHangDispMulti) ? ngayCoHangDispMulti : (r.NgayCoHang != null ? ((DateTime)r.NgayCoHang).ToString("dd/MM/yyyy") : "");
                    if (!string.IsNullOrEmpty(ngayCoHangDispMulti) && ngayCoHangDispMulti.Contains('\n'))
                        worksheet.Cells[row, 15].Style.WrapText = true;
                    worksheet.Cells[row, 16].Value = r.TrangThai ?? "";
                    worksheet.Cells[row, 17].Value = r.GhiChu ?? "";
                    worksheet.Cells[row, 18].Value = r.NgayDuyet != null ? ((DateTime)r.NgayDuyet).ToString("dd/MM/yyyy HH:mm:ss") : "";
                ...
                return File(
                    ...
                    $"Yeu_cau_vat_tu_gop_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx"
                );
            }
        }
        ...
        public IActionResult CapNhatNgayThanhToan(string MaMuahang, string MaSanpham, string? NgayThanhToan)
        {
            try
            {
                ...
                // Lưu vào trường riêng cho Giám đốc
                if (string.IsNullOrWhiteSpace(NgayThanhToan))
                {
                    vt.NgayThanhToanGiamdoc = null;
                }
                else
                {
                    if (DateTime.TryParse(NgayThanhToan, out var dt))
                    {
                        vt.NgayThanhToanGiamdoc = dt;
                    }
                    else
                    {
        ...
        public IActionResult CapNhatGhiChuPhieumuahang(string MaMuahang, string MaSanpham, string? GhiChu)
        {
            try
            {
                ...
                // Lưu vào trường riêng cho Giám đốc
                vt.GhiChuGiamdoc = string.IsNullOrWhiteSpace(GhiChu) ? null : GhiChu.Trim();
            ...
            try
            {
                ...
                if (action == "approve")
                {
                    ...
                    if (yeucau != null && !string.IsNullOrWhiteSpace(yeucau.YCMaDuan) && !string.IsNullOrWhiteSpace(vatTu.MaSanpham))
                    {
                        ...
                        // ⭐ SỐ LƯỢNG ĐÃ CẤP = Số lượng thực tế đã xuất từ phiếu xuất kho
                        // Ưu tiên số lượng từ phiếu xuất kho (số lượng thực tế đã xuất)
                        // Chỉ dùng số lượng từ vtyeucau khi chưa có phiếu xuất kho (fallback)
                        int soLuongDaCap = soLuongDaCapTuPhieuXuat > 0 
                            ? soLuongDaCapTuPhieuXuat 
                            : soLuongDaCapTuVTYeucau;
        ...
        public IActionResult XuLyVatTuYeucauWithCheckbox(string MaYeucau, string VatTuData)
        {
            try
            {
                ...
                bool isNhapKhoRequest =
                    ...
                    // Nếu mã mới (ví dụ 251202NK 260128) không có prefix, phát hiện qua bảng phiếu nhập kho
                    || _context.phieunhapkho.Any(p => p.MaYeucau == MaYeucau)
                    ...
                    if (isApproved)
                    {
                        ...
                        else
                        {
                                ...
                                !string.IsNullOrWhiteSpace(vatTu.MaSanpham))
                            {
                                ...
                                // ⚠️ XỬ LÝ TRƯỜNG HỢP: Số lượng mới < số lượng đã cấp → cần nhập kho trả lại
                                if (soLuongYeuCauHienTai < soLuongDaCap && soLuongDaCap > 0)
                                {
                                    int soLuongThua = soLuongDaCap - soLuongYeuCauHienTai;
                                    Console.WriteLine($"⚠️ [XuLyVatTuYeucauWithCheckbox] Số lượng yêu cầu mới ({soLuongYeuCauHienTai}) nhỏ hơn số lượng đã cấp ({soLuongDaCap}). Cần nhập kho trả lại {soLuongThua} cái cho vật tư {vatTu.MaSanpham}");

                                    // Đặt trạng thái "Chờ nhập kho" để tạo phiếu nhập kho trả lại
                                    vatTu.TrangThai = "Chờ nhập kho";
                                    _context.vtyeucau.Update(vatTu);
                                    processedCount++;
                                    continue;
                                }
                            ...
                            if (khotongDuAn != null && khotongDuAn.SL > 0)
                            {
                                ...
                                // ⭐ XỬ LÝ TRƯỜNG HỢP TĂNG SỐ LƯỢNG KHI KHO ĐỦ HÀNG
                                // Tính số lượng thiếu dựa trên số lượng mới và số lượng đã cấp
                                int soLuongYeuCauHienTai = vatTu.SLMoi ?? vatTu.SL ?? 0;
                                int soLuongThieuCanXu = 0;
                                
                                // Nếu có tính số lượng đã cấp từ logic trên (yêu cầu dự án cùng base code)
                                if (yeucauCheckbox != null &&
                                    !string.IsNullOrWhiteSpace(yeucauCheckbox.YCMaDuan) &&
                                    !string.IsNullOrWhiteSpace(vatTu.MaSanpham))
                                {
                                    ...
                                    var danhSachVTDaNhapTra = _context.vtphieunhapkho
                                        .Where(vt => vt.TrangThai == "Đã nhập kho")
                                        .ToList();
                                    int soLuongDaCap = TinhSoLuongDaCapThucTe(
                                        maYeuCauChuan,
                                        vatTu.MaSanpham ?? "",
                                        danhSachVTDaXuatHopLe,
                                        danhSachVTDaNhapTra);
                                    ...
                                    // ⚠️ XỬ LÝ TRƯỜNG HỢP: Số lượng mới < số lượng đã cấp → cần nhập kho trả lại
                                    if (soLuongYeuCauHienTai < soLuongDaCap)
                                    {
                                        int soLuongThua = soLuongDaCap - soLuongYeuCauHienTai;
                                        Console.WriteLine($"⚠️ [XuLyVatTuYeucauWithCheckbox] Số lượng yêu cầu mới ({soLuongYeuCauHienTai}) nhỏ hơn số lượng đã cấp ({soLuongDaCap}). Cần nhập kho trả lại {soLuongThua} cái cho vật tư {vatTu.MaSanpham}");
                                    ...
                                    // Số lượng thiếu cần xuất = số lượng mới - số lượng đã cấp
                                    soLuongThieuCanXu = soLuongYeuCauHienTai - soLuongDaCap;
                                    
                                    Console.WriteLine($"[DEBUG XuLyVatTuYeucauWithCheckbox] Vật tư {vatTu.MaSanpham}: SL_moi={soLuongYeuCauHienTai}, SL_da_cap={soLuongDaCap}, SL_thieu={soLuongThieuCanXu}, Tồn kho={khotongDuAn.SL}, Khả dụng={soLuongKhaDung}");
                                }
                                else
                                {
                                    // Yêu cầu cá nhân hoặc yêu cầu đầu tiên: dùng số lượng yêu cầu trực tiếp
                                    soLuongThieuCanXu = soLuongYeuCauHienTai;
                                }
                                
                                // Kiểm tra tồn kho có đủ để xuất thêm số lượng thiếu không
                                if (soLuongKhaDung >= soLuongThieuCanXu && soLuongThieuCanXu > 0)
                                {
                                    vatTu.TrangThai = "Chờ xuất kho";
                                }
                                else if (soLuongThieuCanXu > 0)
                                {
                                    // Có số lượng thiếu nhưng kho không đủ → cần mua hàng
                                    vatTu.TrangThai = "Đang mua hàng";
                                }
                                else if (soLuongThieuCanXu == 0 && soLuongYeuCauHienTai > 0)
                                {
                                    // Số lượng mới = số lượng đã cấp → hoàn thành
                                    vatTu.TrangThai = "Hoàn thành";
                                }
                                else
                                {
                                    // Trường hợp khác: giữ nguyên trạng thái hiện tại hoặc đặt mặc định
                                    if (string.IsNullOrWhiteSpace(vatTu.TrangThai))
                                    {
                                        vatTu.TrangThai = "Chờ xuất kho";
                                    }
                            ...
                            {
                                // Không có trong kho tổng, cần kiểm tra số lượng thiếu
                                int soLuongYeuCauHienTai = vatTu.SLMoi ?? vatTu.SL ?? 0;
                                    ...
                                    !string.IsNullOrWhiteSpace(vatTu.MaSanpham))
                                {
                                    ...
                                    {
                                        int soLuongThua = soLuongDaCap - soLuongYeuCauHienTai;
                                        Console.WriteLine($"⚠️ [XuLyVatTuYeucauWithCheckbox] Số lượng yêu cầu mới ({soLuongYeuCauHienTai}) nhỏ hơn số lượng đã cấp ({soLuongDaCap}). Cần nhập kho trả lại {soLuongThua} cái cho vật tư {vatTu.MaSanpham} (không có trong kho)");
                                    ...
                                    int soLuongThieuCanMua = soLuongYeuCauHienTai - soLuongDaCap;
                                    
                                    if (soLuongThieuCanMua > 0)
                                    {
                                        vatTu.TrangThai = "Đang mua hàng";
                                    }
                                    else if (soLuongYeuCauHienTai == soLuongDaCap && soLuongDaCap > 0)
                                    {
                                        vatTu.TrangThai = "Hoàn thành";
                                    }
                                    else
                                ...
                                {
                                    // Yêu cầu cá nhân hoặc yêu cầu đầu tiên: cần mua hàng
                                    vatTu.TrangThai = "Đang mua hàng";
                                }
                            }
                            _context.vtyeucau.Update(vatTu);
                ...
                if (yeucau != null)
                {
                    if (isNhapKhoRequest)
                    {
                        ...
                        if (approvedVTNhap.Any())
                        {
                            ...
                            {
                                // Tạo mã phiếu nhập kho bằng service
                                maNhapkhoToUse = _phieuCodeService.GenerateMaNhapKho(yeucau.YCMaDuan, MaYeucau);
                    ...
                    else
                    {
                        ...
                        // ⚠️ XỬ LÝ VẬT TƯ CẦN NHẬP KHO TRẢ LẠI (số lượng mới < số lượng đã cấp)
                        var vatTuCanNhapTra = vatTuListAfter.Where(v => v.TrangThai == "Chờ nhập kho").ToList();
                        if (vatTuCanNhapTra.Any())
                        {
                            System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] vatTuCanNhapTra.Count={vatTuCanNhapTra.Count}, MaYeucauGoc={MaYeucau}");
                            // Tạo phiếu nhập kho trả lại cho các vật tư có số lượng thừa
                            string maYeuCauChuan = NormalizeMaYeucauBase(MaYeucau);
                            var thongTinYeuCau = yeucau ?? _context.yeucau.FirstOrDefault(y => y.MaYeucau == MaYeucau);
                            
                            if (thongTinYeuCau != null)
                            {
                                // Mã yêu cầu nhập kho: MãDựÁnNK YYMMDD-01 hoặc MãNhânViênNK YYMMDD-01 (PhieuCodeService)
                                var maNvSession = HttpContext.Session.GetString("MaNguoidung");
                                string maYeucauDacBiet = _phieuCodeService.GenerateMaYeucauNhapKho(thongTinYeuCau.YCMaDuan, thongTinYeuCau.YCMaNguoidung ?? maNvSession);

                                var nguoiTraHangInfo = _context.nguoidungs.FirstOrDefault(n => n.MaNguoidung == thongTinYeuCau.YCMaNguoidung);
                                // Luồng này được tạo ra từ hành động "Giám đốc duyệt" (giảm SL mới < SL đã cấp),
                                // nên không cần qua bước duyệt nữa. Đưa thẳng về Kho xử lý.
                                ...
                                var newYeucauDacBiet = new yeucau
                                {
                                    ...
                                    YCMaNguoidung = thongTinYeuCau.YCMaNguoidung,
                                    NguoiYeucau = nguoiTraHangInfo?.TenNguoidung ?? "",
                                ...
                                // Mã phiếu nhập kho: MãDựÁnNK YYMMDD-01 hoặc MãNhânViênNK YYMMDD-01
                                string maNhapkhoTra = _phieuCodeService.GenerateMaNhapKho(thongTinYeuCau.YCMaDuan, MaYeucau);

                                // Tạo phiếu nhập kho hoàn trả: không cần chờ duyệt nữa, chuyển thẳng "Chờ nhập kho"
                                ...
                                _context.SaveChanges();
                                System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] Created phieunhapkho return: MaNhapkho={maNhapkhoTra}, MaYeucauNK={maYeucauDacBiet}, TrangThai={trangThaiPhieuNhap}");

                                // Thêm vật tư vào phiếu nhập kho hoàn trả
                                foreach (var vtTra in vatTuCanNhapTra)
                                {
                                    // Tính số lượng thừa cần trả lại
                                    string maYeuCauChuanVT = NormalizeMaYeucauBase(MaYeucau);
                                    var trangThaiDaCap = new[]
                                    {                                        
                                    ...
                                    int soLuongYeuCauHienTai = vtTra.SLMoi ?? vtTra.SL ?? 0;
                                    int soLuongThua = Math.Max(0, soLuongDaCap - soLuongYeuCauHienTai);
                                    System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] Return calc: MaSP={vtTra.MaSanpham}, SLMoi/SL={soLuongYeuCauHienTai}, soLuongDaCap={soLuongDaCap}, soLuongThua={soLuongThua}, MaKho={vtTra.YCMakho}");

                                    // Không có số lượng thừa thì không tạo dòng nhập kho trả lại
                                    if (soLuongThua <= 0)
                                    {
                                        System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] Skip add vtphieunhapkho (soLuongThua<=0). MaSP={vtTra.MaSanpham}");
                                        continue;
                                    }
                                    ...
                                    var vtPhieuNhap = new vtphieunhapkho
                                    {
                                        MaNhapkho = maNhapkhoTra,
                                        MaYeucau = maYeucauDacBiet,
                                        TenSanpham = vtTra.TenSanpham,
                                        MaSanpham = vtTra.MaSanpham,
                                        Makho = vtTra.YCMakho,
                                        HangSX = vtTra.HangSX,
                                        NhaCC = vtTra.NhaCC,
                                        DonVi = vtTra.DonVi,
                                        DiengiaiNhapKho = "Giảm số lượng yêu cầu",
                                        NgayBaohanh = vtTra.NgayBaohanh,
                                        ThoiGianBH = vtTra.ThoiGianBH,
                                        SL = soLuongThua,
                                        TrangThai = trangThaiPhieuNhap
                                    };
                                    _context.vtphieunhapkho.Add(vtPhieuNhap);
                                    System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] Added vtphieunhapkho: MaNhapkho={maNhapkhoTra}, MaSP={vtTra.MaSanpham}, SL={soLuongThua}");
                                }
                                _context.SaveChanges();

                                var vtCountAfter = _context.vtphieunhapkho.Count(vt => vt.MaNhapkho == maNhapkhoTra);
                                System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] vtphieunhapkho rows after save: MaNhapkho={maNhapkhoTra}, Count={vtCountAfter}");
                            }
                            else
                            {
                                System.Diagnostics.Debug.WriteLine($"[Giamdoc/XuLyVatTuYeucauWithCheckbox] thongTinYeuCau is null. MaYeucau={MaYeucau}");
                            }
                        }
                                           ...
                                           List<string> DonVi, string MaYeucau, string action, phieuxuatkho phieuxuatkho, vtphieuxuatkho vtphieuxuatkho, phieumuahang phieumuahang, vtphieumuahang vtphieumuahang)
        {
            ...
            {
                // ===== YÊU CẦU NHẬP KHO (đã tồn tại) – cập nhật trạng thái & số lượng, rồi tạo mã phiếu nhập kho =====
                // 1) Lấy yêu cầu thực tế trong DB theo MaYeucau được truyền vào
                var ycDb = _context.yeucau.FirstOrDefault(x => x.MaYeucau == MaYeucau);
                if (ycDb != null)
                {
                    // 2) Nếu có thay đổi số lượng (SLMoi > 0) thì đưa trạng thái yêu cầu về "Chờ nhập kho"
                    bool coThayDoiSL = SLMoi != null && SLMoi.Any(x => x.HasValue && x.Value > 0);
                    if (coThayDoiSL)
                    {
                        ycDb.TrangThai = "Chờ nhập kho";
                        // Nếu cần, có thể cập nhật lại NgayYeucau để đánh dấu lần cập nhật gần nhất
                        // ycDb.NgayYeucau = DateTime.Now;
                    }

                    // 3) Cập nhật lại SLMoi/SL cho từng dòng vật tư của yêu cầu dựa theo MaSanpham
                    if (MaSanpham != null && SLMoi != null)
                    {
                        for (int i = 0; i < MaSanpham.Count; i++)
                        {
                            var maVt = MaSanpham[i];
                            var slMoi = (i < SLMoi.Count) ? SLMoi[i] : null;
                            if (string.IsNullOrWhiteSpace(maVt) || !slMoi.HasValue)
                            {
                                continue;
                            }

                            var vtDb = _context.vtyeucau.FirstOrDefault(v =>
                                v.VTMaYeucau == ycDb.MaYeucau &&
                                v.MaSanpham == maVt);

                            if (vtDb != null)
                            {
                                vtDb.SLMoi = slMoi;
                                vtDb.SL = slMoi.Value; // SL thực tế sau điều chỉnh
                            }
                        }
                    }
                    ...
                    // 4) Tạo mã phiếu nhập kho gắn với chính MaYeucau của yêu cầu này
                    phieunhapkho.MaNhapkho = _phieuCodeService.GenerateMaNhapKho(phieunhapkho.MaDuan, ycDb.MaYeucau);
                }
                else
                {
                    // Không tìm thấy yêu cầu tương ứng, fallback: vẫn tạo mã phiếu nhập kho như cũ
                                ...
                                vtyeucau vtyeucau)
        {
            ...
            if (danhSachVatTuCanNhapTra.Any())
            {
                ...
                // Mã yêu cầu nhập kho: MãDựÁnNK YYMMDD-01 hoặc MãNhânViênNK YYMMDD-01 (PhieuCodeService)
                string maYeucauDacBiet = _phieuCodeService.GenerateMaYeucauNhapKho(thongTinYeuCau.YCMaDuan, maNguoiTraHang);
                ...
                if (!string.IsNullOrEmpty(thongTinYeuCau.YCMaDuan))
                {
                    ...
                    bool laNguoiThuocDuAn = (!string.IsNullOrEmpty(boPhanNguoiTra) && boPhanNguoiTra.Trim().Equals("BP dự án", StringComparison.OrdinalIgnoreCase)) ||
                        (!string.IsNullOrEmpty(chucVuNguoiTra) && (chucVuNguoiTra.Trim().Equals("Quản lí dự án", StringComparison.OrdinalIgnoreCase) || chucVuNguoiTra.Trim().Equals("Quản lý dự án", StringComparison.OrdinalIgnoreCase))) ||
                        (duan != null && !string.IsNullOrEmpty(duan.MaNguoiQLDA) && maNguoiTraHang.Trim().Equals(duan.MaNguoiQLDA.Trim(), StringComparison.OrdinalIgnoreCase));
                    trangThaiYeucau = laNguoiThuocDuAn ? "Chờ giám đốc duyệt" : "Chờ quản lý dự án duyệt";
                }
                ...
                string maNhapkhoTra = _phieuCodeService.GenerateMaNhapKho(thongTinYeuCau.YCMaDuan, maYeucauDacBiet);

                // Tạo phiếu nhập kho hoàn trả
                ...
                foreach (var vtTra in danhSachVatTuCanNhapTra)
                {
                    var vtPhieuNhap = new vtphieunhapkho
                    {
                        ...
                        DiengiaiNhapKho = "Giảm số lượng yêu cầu", // Lý do: Chênh lệch số lượng yêu cầu
        ...
        private void XuliphieuyeucauPartial(string Mayeucau, List<string> approvedMaSanphamList)
        {
            ...
            if (laYeuCauCaNhan || existingPhieuXuatKho == null)
            {
                ...
                Maxuatkho = _phieuCodeService.GenerateMaXuatKho(thongTinYeuCau.YCMaDuan, Mayeucau);
            }
            else
            {
            ...
            // Nếu chưa có phiếu mua hàng, tạo mã mới bằng service
            if (existingPhieuMuaHang == null)
            {
                Mamuahang = _phieuCodeService.GenerateMaMuaHang(thongTinYeuCau.YCMaDuan, Mayeucau);
            }
            else
            {
            ...
            foreach (var VattuYC in danhSachVatTuYCSapXep)
            {
                ...
                if (khotong != null && khotong.SL > 0)
                {
                    ...
                    // ⚠️ QUAN TRỌNG: Tạo keyKhoSanPham dựa trên khotong thực tế được tìm thấy
                    // Nếu khotong được tìm thấy theo YCMakho thì dùng YCMakho, nếu không thì dùng Makho của khotong
                    string makhoForKey = !string.IsNullOrEmpty(VattuYC.YCMakho) && khotong.Makho == VattuYC.YCMakho 
                        ? VattuYC.YCMakho 
                        : (khotong.Makho ?? "");
                    string keyKhoSanPham = $"{makhoForKey}_{khotong.MaSanpham ?? ""}";
                    int soLuongDaCamKetTrongCungYeuCau = soLuongDaCamKetTrongYeuCau.GetValueOrDefault(keyKhoSanPham, 0);
                    
                    // ⚠️ DEBUG: Log để kiểm tra
                    Console.WriteLine($"[DEBUG XuliphieuyeucauPartial] Vật tư {VattuYC.MaSanpham}: YCMakho={VattuYC.YCMakho}, khotong.Makho={khotong.Makho}, keyKhoSanPham={keyKhoSanPham}, soLuongDaCamKetTrongCungYeuCau={soLuongDaCamKetTrongCungYeuCau}");
                    ...
                    if (coNhieuYeuCauCungBaseCode)
                    {
                        ...
                        else
                        {
                            ...
                            // ⚠️ SỬA LỖI: Luôn tính số lượng xuất dựa trên số lượng khả dụng thực tế trong kho
                            // Không được xuất quá số lượng khả dụng, kể cả khi trạng thái là "Chờ xuất kho" hoặc "Đã xuất kho"
                            // Vì trạng thái này chỉ cho biết vật tư đã được duyệt để xuất kho, nhưng số lượng xuất phải dựa trên tồn kho thực tế
                    ...
                    else
                    {
                        ...
                        Console.WriteLine($"[DEBUG XuliphieuyeucauPartial] Vật tư {VattuYC.MaSanpham}: Tồn kho={khotong.SL}, Đã cam kết từ yêu cầu khác={soLuongDaCamKetTuYeuCauKhac}, Đã cam kết trong cùng yêu cầu={soLuongDaCamKetTrongCungYeuCau}, Tổng đã cam kết={tongSoLuongDaCamKet}, Khả dụng={soLuongKhaDung}, Yêu cầu={soLuongYeuCauThucTe}, Key={keyKhoSanPham}");
                        ...
                        Console.WriteLine($"[DEBUG XuliphieuyeucauPartial] Vật tư {VattuYC.MaSanpham}: Số lượng xuất={soLuongXuat}, Số lượng thiếu={soLuongThieu}");
                    }
                ...
                else
                {
                    ...
                    if (isPhieuMuaHangCreated)
                    {
                        ...
                        if (VattuYC.TrangThai != "Hoàn thành")
                        {
                            // ⚠️ SỬA LỖI: Tính số lượng mua hàng dựa trên số lượng yêu cầu thực tế
                            // Nếu có nhiều yêu cầu cùng base code, tính số lượng cần cấp thêm
                            int soLuongMuaHang = VattuYC.SL ?? 0;
                            ...
                            if (coNhieuYeuCauCungBaseCode)
                            {
                                ...
                                // Số lượng cần cấp thêm = MAX hiện tại - Số đã cấp
                                int soLuongYeuCauHienTai = Math.Max(VattuYC.SLMoi ?? 0, VattuYC.SL ?? 0);
                                soLuongMuaHang = Math.Max(0, Math.Max(maxHienTai, soLuongYeuCauHienTai) - soLuongDaCap);
                            }
        ...
        private int TinhSoLuongDaCamKet(string makho, string masanpham, DateTime? ngayDuyetHienTai = null, string maXuatkhoHienTai = null)
        {
            ...
            // ⚠️ DEBUG: Log để kiểm tra
            Console.WriteLine($"[DEBUG TinhSoLuongDaCamKet] makho={makho}, masanpham={masanpham}, ngayDuyetHienTai={ngayDuyetHienTai}, maXuatkhoHienTai={maXuatkhoHienTai}, tongSoLuongDaCamKet từ phiếu xuất={tongSoLuongDaCamKet}");
            ...
            int[] SL, string[] DonVi, string[] Makho, string LoaiNhapkho, decimal[] DonGia, string[] DiengiaiNhapKho)
        {
            ...
            try
            {
                ...
                // Mã yêu cầu nhập kho: MãDựÁnNK YYMMDD-01 hoặc MãNhânViênNK YYMMDD-01 (PhieuCodeService)
                    ...
                    string maNguoiDungForYc = (LoaiNhapkho == "duan" && !string.IsNullOrEmpty(phieunhapkho.MaDuan)) ? maNguoiYeuCauBanDau : maNv;
                    string maYeucauDacBiet = _phieuCodeService.GenerateMaYeucauNhapKho(maDuanForYc, maNguoiDungForYc);
                    ...
                    var newYeucauDacBiet = new yeucau
                    {
                        MaYeucau = maYeucauDacBiet,
                        TenYeucau = "Yêu cầu nhập kho",
                        YCMaNguoidung = maNguoiYeuCauBanDau,
                ...
                try
                {
                    ...
                    _ = System.Threading.Tasks.Task.Run(async () =>
                    {
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine($"[Giamdoc/ThemPhieunhapkhoSQL] Lỗi gửi email tạo phiếu nhập kho: {exInner.Message}");
                        }
                    });
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[Giamdoc/ThemPhieunhapkhoSQL] Lỗi khởi chạy task gửi email: {ex.Message}");
                }
            ...
            if (submitAction == "approve")
            {
                    ...
                    _ = Task.Run(async () =>
                    {
                        try
                        {
                            using (var scope = _serviceScopeFactory.CreateScope())
                            {
                                ...
                                await emailService.SendNotificationToRequesterOnNhapKhoStatusAsync(maNhapkhoForEmail, "Giám đốc đã duyệt, chờ nhập kho");
                            }
                        }
                        catch (Exception exInner)
                        {
                ...
                else if (Phieunhapkho.TrangThai == "Chờ nhập kho" && boPhan2 == "BP kho")
                {
                    ...
                    foreach (var maYc in maYeucauList)
                    {
                        ...
                        if (yeuCau != null)
                        {
                            // Kiểm tra xem có phải là yêu cầu nhập kho hoàn trả hay không
                            // - Hỗ trợ cả mã cũ (NHAPKHO_CANHAN_, NHAPKHO_DUAN_) và mã mới (ví dụ: MaNhanVienNK YYMMDD-01)
                            bool isNhapKhoRequest =
                                (!string.IsNullOrEmpty(maYc) &&
                                    (maYc.StartsWith("NHAPKHO_CANHAN_", StringComparison.OrdinalIgnoreCase) ||
                                     ...
                                     maYc.StartsWith("NHAPKHO_", StringComparison.OrdinalIgnoreCase)))
                                || _context.phieunhapkho.Any(p => p.MaYeucau == maYc)
                                || _context.yeucau.Any(y => y.MaYeucau == maYc && y.TenYeucau == "Yêu cầu nhập kho");
                    ...
                    if (!string.IsNullOrEmpty(Phieunhapkho.MaYeucau) && !isNhapKhoOnlyFlow)
                    {
                        ...
                        if (vatTuChuaXuat.Any())
                        {
                            ...
                            if (phieuXuatLienQuan == null)
                            {
                                ...
                                string MaXuatkho = _phieuCodeService.GenerateMaXuatKho(yeucau?.YCMaDuan, Phieunhapkho.MaYeucau);
