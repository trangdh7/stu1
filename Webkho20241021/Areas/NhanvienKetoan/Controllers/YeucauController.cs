...
namespace Webkho_20241021.Areas.NhanvienKetoan.Controllers
{
    ...
    public class YeucauController : Controller
    {
        ...
        public YeucauController(ApplicationDbContext context, EmailService emailService, IYeucauCodeService yeucauCodeService, IPhieuCodeService phieuCodeService)
        {
            _context = context;
                ...
                else
                {
                    ...
                    if (isNhapKhoRequest)
                    {
                        var rawNk = (from vtnk in _context.vtphieunhapkho
                                     ...
                                     select new { vtnk, MaSanpham = vtnk.MaSanpham, SLMoi = vtnk.SL }).ToList();
                        ...
                        vatTuList = rawNk.Select(x =>
                        {
                            ...
                            return new { v.ID, VTMaYeucau = MaYeucau, v.TenSanpham, v.MaSanpham, YCMakho = v.Makho, v.HangSX, v.NhaCC, SLCu = (int?)null, SLMoi = v.SL, SL = v.SL, v.DonVi, NgayCanHang = (DateTime?)null, v.NgayNhapkho, v.NgayBaohanh, v.ThoiGianBH, NgayDuyet = (DateTime?)null, v.TrangThai, GhiChu = (string?)null, TonKho = tonKho, SlThieu = Math.Max(0, (x.SLMoi ?? 0) - tonKho), SlDaXuat = (int?)null };
                        }).ToList<object>();
                    }
                    else
                    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    _ = System.Threading.Tasks.Task.Run(async () =>
                    {
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine($"[NhanvienKetoan/ThemPhieunhapkhoSQL] Lỗi gửi email tạo phiếu nhập kho: {exInner.Message}");
                        }
                    });
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[NhanvienKetoan/ThemPhieunhapkhoSQL] Lỗi khởi chạy task gửi email: {ex.Message}");
                }
